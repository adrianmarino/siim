class MedicalHistoryStringResultBuilder
	...
	def patient_firstname
		value_of "patient.firstname.title", @medical_history.patient.firstname
	end

	def patient_lastname
		value_of "patient.lastname.title", @medical_history.patient.lastname
	end

	def patient_birthdate
		value_of "patient.birthdate.title", @medical_history.patient.birthdate
	end

	def patient_height
		value_of "patient.height.title", @medical_history.patient.height
	end

	def patient_weight
		value_of "patient.weight.title", @medical_history.patient.weight
	end

	def allergy_cause(an_model)
		value_of "allergies.cause.title", an_model.cause
	end

	def allergy_observations(an_model)
		value_of "allergies.observations.title", an_model.observations
	end

	def consultation_treatment(a_model)
		value_of "consultations.treatment.title", a_model.treatment
	end

	def consultation_symptomps(a_model)
		value_of "consultations.symptomps.title", a_model.symptomps
	end

	def consultation_diagnostic(a_model)
		value_of "consultations.diagnostic.title", a_model.diagnostic
	end

	def disease_name(a_model)
		value_of "diseases.name.title", a_model.name
	end

	def disease_observations(a_model)
		value_of "diseases.observations.title", a_model.observations
	end

	def medication_name(a_model)
		value_of "medications.name.title", a_model.name
	end

	def medication_dose(a_model)
		value_of "medications.dose.title", a_model.dose
	end

	def medication_amount(a_model)
		value_of "medications.amount.title", a_model.amount
	end

	def medication_route(a_model)
		value_of "medications.route.title", a_model.route
	end

	def exam_name(a_model)
		value_of "medical_exams.name.title", a_model.name
	end

	def exam_results(a_model)
		value_of "medical_exams.results.title", a_model.results
	end

	def exam_observations(a_model)
		value_of "medical_exams.observations.title", a_model.observations
	end
