class MedicalHistoryStringResultBuilder
	...
	# -------------------------------------------------------------------------
	def with_patient
		@result+= "#{patient_firstname}#{SEPARATOR}#{patient_lastname}#{SEPARATOR}#{patient_birthdate}#{SEPARATOR}#{patient_height}#{SEPARATOR}#{patient_weight}"
		self
	end

	def with_allergies
		@result = @medical_history.allergies.inject(@result) {|result, an_allergy |
			result+=  "#{SEPARATOR}#{allergy_cause(an_allergy)}"
			result+= "#{SEPARATOR}#{allergy_observations(an_allergy)}" if !an_allergy.observations.nil? and !an_allergy.observations.empty?
			result
		}
		self
	end
	...
	def with_antecedents
		@result = @medical_history.antecedents.inject(@result) {|result, an_antecedent | "#{result}#{SEPARATOR}#{antecedents_description(an_antecedent)}"}
		self
	end

	def with_consultations
		@result = @medical_history.consultations.inject(@result) {|result, a_consultation |
			result+=  " #{SEPARATOR}#{consultation_diagnostic(a_consultation)}"
			result+= "#{SEPARATOR}#{consultation_symptomps(a_consultation)}" unless a_consultation.symptomps.nil? or a_consultation.symptomps.empty?
			result+= "#{SEPARATOR}#{consultation_treatment(a_consultation)}" unless a_consultation.treatment.nil? or a_consultation.treatment.empty?
			result
		}
		self
	end
	...
	def with_diseases
		@result = @medical_history.diseases.inject(@result) {|result, a_disease |
			result+= "#{SEPARATOR}#{disease_name(a_disease)}"
			result+= "#{SEPARATOR}#{disease_observations(a_disease)}" unless a_disease.observations.nil? or a_disease.observations.empty?
			result
		}
		self
	end
	...
	def with_medications
		@result = @medical_history.medications.inject(@result) {|result, a_medication |
			result+= "#{SEPARATOR}#{medication_name(a_medication)}#{SEPARATOR}#{medication_dose(a_medication)}#{SEPARATOR}#{medication_amount(a_medication)}"
			result+= "#{SEPARATOR}#{medication_route(a_medication)}" unless a_medication.route.nil? or a_medication.route.empty?
			result
		}
		self
	end
	...
	def with_vaccines
		@result = @medical_history.vaccines.inject(@result) {|result, a_vaccine |"#{result}#{SEPARATOR}#{vaccines_name(a_vaccine)}"}
		self
	end

	def with_exams
		@result = @medical_history.medical_exams.inject(@result) {|result, an_exam |
			result+= "#{SEPARATOR}#{exam_name(an_exam)}#{SEPARATOR}#{exam_results(an_exam)}"
			result+= "#{SEPARATOR}#{exam_observations(an_exam)}" unless an_exam.observations.nil? or an_exam.observations.empty?
			result
		}
		self
	end
	...
	def antecedents_description(a_model)
		value_of "antecedents.description", a_model.description
	end

	def vaccines_name(a_model)
		value_of "vaccines.name", a_model.name
	end
	...
	def initialize(a_medical_history, indexes)
		...
		@result = ""
		@highlight_wrapper  = HighlightWrapper.new_with_default_tags @medical_history.highlight , indexes
	end
